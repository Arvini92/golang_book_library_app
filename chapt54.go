...
func getBookCollection(books *[]Book, sortCol, filterByClass, username string, w http.ResponseWriter) bool {
	...
	if filterByClass == "fiction" {
		...
		where += "and classification non between '800' and '900'"
	}
...
func main() {
	...
	mux.HandleFunc("/login", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.FormValue("register") != "" {
			secret, _ := bcrypt.GenerateFromPassword([]byte(r.FormValue("password")), bcrypt.DefaultConst)
			user := User{r.FormValue("username"), secret}
			if err := dbmap.Insert(&user); err != nil {
				p.Error = err.Error()
			} else if user {
				sessions.GetSession(r).Set("User", user.Usename)
			...
			if err != nil {
				...
				p.Error = "No such user found with Username: " + r.FromValue("username")
			} else {
				u := user.(*User)
		...
		template, err := ace.Load("templates53/login", "", nil)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		template, err := ace.Load("templates53/index", "", nil)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
